...
export const MoreScreen = () => {
  ...
  const menuItems = [
    ...
    {
      title: 'Reportes y Equipo',
      items: [
        { label: 'Rendimiento de Equipo', icon: TrendingUp, screen: 'StaffPerformance', color: '#f59e0b' },
        { label: 'Usuarios y Roles', icon: Users, screen: null, color: '#8b5cf6' }, // Future
      ]
    },
    {
      title: 'Cuenta y Suscripción',
      items: [
        { label: 'Mi Suscripción', icon: TrendingUp, screen: 'Subscription', color: '#f59e0b' },
      ]
    },
    {
      title: 'Configuracion',
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <AestheticHeader title="Mi Negocio" />

      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Surface style={styles.profileCard} elevation={2}>
            <Avatar.Text
                size={60}
                label={user?.nombre_completo?.substring(0,2).toUpperCase() || 'U'}
                style={{ backgroundColor: '#0f172a' }}
            />
            <View style={{ marginLeft: 16 }}>
                <Text style={styles.profileName}>{user?.nombre_completo || 'Administrador'}</Text>
                <View style={styles.locationTag}>
                    <MapPin size={12} color="#64748b" />
                    <Text style={styles.locationText}>Sede Principal</Text>
                </View>
            </View>
        </Surface>
